...
<style type="text/css">
	...
	#login_header{width:600px; font-size: 60px;   font-weight: bold; display: flex;  justify-content: center; align-items: center;}
	#login_container{width: 700px; display: flex; flex-direction: column; justify-content: center; align-items: center;}
	#login_font{width: 500px; height: 140px; font-size: 20px;   margin-right: 800px;}
...
function chkNull(){
	...
	function idFindSuccess() {
		window.open("http://localhost/ss/login/idFindSuccess.do?id=${ idFind.member_id }&name=${ idFind.member_name }&signDate=${ idFind.member_sign_Date }","success","width = 640, height = 387, top ="+(window.sceenTop+350)+",left =" +(window.creenleft + 600));
	} 	
...
<body>
		...
		<div  id = container>
			<div class = main_con >
			<div id=login_font><p>ID FIND</p>홈 > 아이디 찾기</div>
			<div id = login_wrap>
				<div id="login_header">아이디 찾기</div><br/>
				<form action="http://localhost/ss/login/idSearch.do" name = "findFrm" id = "findFrm" method="post">
				<div id= login_container>
					<label>
						<span id = "spanId">이름</span>
						<input type="text" autofocus="autofocus" tabindex="1" maxlength="20" name="member_name" id ="name">
					</label>
					<br/>
					<label>
						<span id = "spanPw">이메일</span>
						<input type="text"  tabindex="2" maxlength="30" name ="member_email" id ="email">
					</label>
					<br/>
					<div  id = "guide">
