...
window.onload = function(){
	...
	var birth = "${ memberInfo.member_birth }";
	var phone = "${ memberInfo.member_phone }";
	...
	obj.phone3.value = phone3; 
	
	//<select> 
	var opcnt = obj.phone1.length;
	
	 for(i = 0; i<opcnt; i++ ){
		 if(obj.phone1[i].value == phone1){
			 obj.phone1[i].selected = true;
		 }
	 }	
}//onload
	...
	function submit(){
		...
		var phone3 = obj.phone3.value;
		obj.member_phone.value = phone1+"-"+phone2+"-"+phone3;
		
		var year = obj.year.value;
		var month = obj.month.value;
		var day = obj.day.value;
		obj.member_birth.value = year+month+day;
	...
	function password(){
		location.href = "http://localhost/ss/login/password.do";
	}
	...
	<c:if test="${ !empty withdrawal }">
		memberDelete();
	</c:if>
	
	function memberDelete() {
		window.open("http://localhost/ss/login/withdrawal_success.do?id=${ id }","withdrawal_success","width = 640, height = 387, top ="+(window.sceenTop+350)+",left =" +(window.creenleft + 600));
	 } 
	...
	function updateSuccess() {
		window.open("http://localhost/ss/login/member_success.do?id=${ id }","password_success","width = 640, height = 387, top ="+(window.sceenTop+350)+",left =" +(window.creenleft + 600));
	 } 	
...
<!-- header end --> 
		<div  id = container>
			...
			<div id=join_font>
				...
				<form action="http://localhost/ss/login/memberUpdate.do" name="joinFrm"  id="joinFrm" method="post">
				<div id = join_basic>
					<table>
						<tr>
						...
						<tr>
							<th>&emsp;이름</th>
							<td>
								 <input type="text" class="form-control text" name="member_name" id = "name" value="${ memberInfo.member_name }">
						...
						<tr style="height: 205px">
							<th>&emsp;주소</th>
							<td>
								<input type="text" class="form-control text zipcode" id="zipcode"  name="member_zipcode" value="${ memberInfo.member_zipcode }">
						...
						<tr>
							<th>&emsp;휴대전화</th>
							<td>	
								<c:set var="phoneValue" value="${fn:split('010,011,016,017,018,019',',')}"/>
								<select class="form-control phoneTx"  id="phone1" name="phone1" >
									<c:forEach var="phoneValue" items="${ phoneValue }" varStatus="i">
										 <option value="${ phoneValue }">
										 	${ phoneValue }
										</option>   
									</c:forEach> 
						...
						<tr>
							<th>&emsp;이메일</th>
							<td>
								 <input type="text" class="form-control text" id="email" name ="member_email" value="${ memberInfo.member_email }"> 
							</td>
						</tr>
					</table>
			</div>
			...
			<div id= join_add>
					<table>
						<tr>
							...
							<td>
								<div class="genderDiv">
  									<input type="radio"  name="member_gender" value="남자" id="Radio1"  class="gender"${ memberInfo.member_gender eq "남자"?"checked='checked'":"" }>
  									<span class="gender">남자</span>&emsp;
									<input type="radio"  name="member_gender" value="여자" id="Radio2"  class="gender"${ memberInfo.member_gender eq "여자"?"checked='checked'":"" }>
					...
					<a onclick="withdrawal()" id="withdraw" >회원탈퇴</a>
